...
export default function MyAppointments() {
  ...
  const fetchAppointments = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/patient/${patient_id}/appointments`);
      if (res.ok) {
        const data = await res.json();
        // FLEXIBLE DATA KEY CHECK
  ...
  useEffect(() => { fetchAppointments(); }, [patient_id]);

  // Helper to normalize dates for comparison (ignoring time)
  const normalizeDate = (d) => {
    const date = new Date(d);
    return isNaN(date.getTime()) ? 0 : date.setHours(0, 0, 0, 0);
  };

  const todayNormalized = new Date().setHours(0, 0, 0, 0);

  // Filter logic applied during render
  const filteredAppointments = rawAppointments.filter(a => {
    const apptDate = normalizeDate(a.preferred_date);
    if (filter === "upcoming") return apptDate >= todayNormalized && a.status?.toLowerCase() !== 'cancelled';
    if (filter === "past") return apptDate < todayNormalized || a.status?.toLowerCase() === 'cancelled';
    return true; // "all"
  });
  ...
  const submitAppointment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setAForm({ department: "", preferred_date: "", preferred_time: "", reason: "" });
        setModal(false);
      ...
      <div className="page">
        ...
        <div className="content">
          <div className="page-header">
            ...
            <p>Managing {rawAppointments.length} total records.</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="appt-list">
              {filteredAppointments.map(a => {
                const apptDate = normalizeDate(a.preferred_date);
                const upcoming = apptDate >= todayNormalized;
                const cancelled = a.status?.toLowerCase() === "cancelled";

                return (
                  <div key={a.appt_id} className={`appt-card ${cancelled ? 'cancelled' : upcoming ? 'upcoming' : 'past'}`}>
                    <div className="appt-body">
                      <div className="appt-main">
                        <div className="appt-dept">{a.department}</div>
                        <div className="appt-chips">
                          {/* Formatting SQL Date to readable string */}
                          <span className="chip">📆 {new Date(a.preferred_date).toDateString()}</span>
                          <span className="chip">🕐 {a.preferred_time}</span>
                          <span className="chip">{upcoming ? "🟢 Upcoming" : "🔵 Past"}</span>
                        </div>
